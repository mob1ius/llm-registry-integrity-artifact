"""Stage-2 behavioral confirmation for a SECOND anchor model (Phi-3-mini-4k
-instruct), to generalize the Mistral finding beyond a single model. Same
mechanism found in the static audit: a minority-cluster template
(qwp4w3hyb and 5 others, spanning both popularity buckets) has NO system-
role branch at all -- unlike Mistral's case, this doesn't even raise an
exception, it silently renders the prompt with the system message dropped
entirely (verified by direct Jinja2 rendering, see chat log).

Full 3-condition design run in one pass this time (the Mistral test needed
a follow-up script to add the missing control arm -- doing it right the
first time here):
  A. minority template, no system  (real-world consequence: system
     content is silently absent regardless of whether the caller tries to
     set it, since the template has no branch that would emit it)
  B. modal template, with system   (system properly delivered via the
     <|system|> token)
  C. modal template, no system     (isolates template-shape alone, holding
     system-prompt-presence constant at "none")

Same 140-prompt HarmBench Standard set (21-prompt stratified subsample),
same REFUSE/COMPLY classifier, same methodology as the Mistral pilot, using
the already-local phi3:3.8b weights via Ollama raw-generate mode.
"""
...
def main():
    ...
    n = len(results)
    a_rate = sum(r["condition_a_minority_no_system_refused"] for r in results) / n
    b_rate = sum(r["condition_b_modal_with_system_refused"] for r in results) / n
